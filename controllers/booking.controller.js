...
module.exports.Update = async (req,res) => {
    ...
    try {
       ...
       Booking.findOneAndUpdate({_id:id},data,{returnOriginal:false},(err,result)=>{
            if(err){
...
module.exports.Accept = async (req,res) =>{
    ...
    Booking.findByIdAndUpdate(id,{status:updatedata},{returnOriginal:false},async (err,accepted)=>{
        if(err){
            console.log(err);
            return res.status(500).send({message:err.message});
        }
...
await axios(config).then(function (response) {
    console.log(JSON.stringify(response.data));
        ...
        console.log(booking.partner_id);
...
module.exports.Reject = async (req,res,next) =>{
    ...
    try {
       ...
       Booking.findOneAndUpdate({_id:booking_id},{status:status,note:req.body.reason},{returnDocument:'after'},async (err,result)=>{
            if(err){
                return res.status(500).send({message:err.message});
            }
