...
module.exports.Update = async (req,res) => {
    ...
    try {
       ...
       RoomStatus.findOneAndUpdate({_id:id},data,{returnOriginal:false},(err,result)=>{
            if(err){
                return res.status(500).send({message:err.message});
            }
...
module.exports.Delete = async (req,res) => {
    try {
      RoomStatus.findOneAndDelete({_id:req.params.id},null,((err,result) =>{
            if(err){
                return res.status(500).send({message:err.message});
            }
